...
Attenzione. Qui si varia perchè dagli eur bisogna eliminare gli italiani e i CEU
...
Qui marco i CEU e elimino i toscani.

```{r Population issue}
# Reducing population from multiple names to 3 parametrized name
# str_detect it's the solution
data.list = lapply(data.list, function(x) mutate(x, Population = ifelse(str_detect(Population, "TSI"), "TSI", ifelse(str_detect(Population, "CEU"), "CEU", Pop.values))))

# Remove TSI
data.list = lapply(data.list, function(x) filter(x, Population != "TSI"))
```
...
# Export for CEU & Removal of CEU from EUR

```{r Export for CEU}
# Subsetting for CEU
CEU.long = data.long %>% filter(Population == "CEU")
CEU.wide = data.wide %>% filter(Population == "CEU")

# Removing CEU from EUR
data.long = data.long %>% filter(Population != "CEU")
data.wide = data.wide %>% filter(Population != "CEU")

# Export in CSV
write.csv(CEU.long, file = paste(geno.path, "Ceu long.csv", sep = ""), row.names = FALSE)
write.csv(CEU.wide, file = paste(geno.path, "Ceu wide.csv", sep = ""), row.names = FALSE)

# Cleaning
remove(CEU.long, CEU.wide)
```
