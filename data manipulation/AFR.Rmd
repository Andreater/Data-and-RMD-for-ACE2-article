...
Niente di particolarmente interessante: Splitto per genere il gruppo di controllo e manipolo gli italiani. Sarebbe stato meglio farlo in un altro foglio e poi importarsi le cose già pronte ma onestamente non mi va. AMR genotipo. TUTTI hanno il problema degli alleli tranne gli AMR

```{r Split for Gender}
Al.females.count = data.long %>% 
  filter(Gender == "F")%>%
  group_by(Location) %>% 
  summarise(CTRL.Al1 = sum(Al.1),
            CTRL.Al2 = sum(Al.2))

Al.males.count = data.long %>% 
  filter(Gender == "M") %>%
  group_by(Location) %>% 
  summarise(CTRL.Al1 = sum(Al.1),
            CTRL.Al2 = sum(Al.2))

Al.gender.list = list("F" = Al.females.count,
                      "M" = Al.males.count)
```
...
```{r}
ITA.f = ITA.Al.females %>% rename(Female.Al1 = Al.1, Female.Al2 = Al.2)
ITA.m = ITA.Al.males %>% rename(Male.Al1 = Al.1, Male.Al2 = Al.2)

ITA = left_join(ITA.f, ITA.m, by = "Location")
...
```{r Allelic fisher in ref ITA}
# Applico la funzione per il Fisher test
res = apply(ITA, MARGIN = 1, FUN = rowfish)

# Traspongo la matrice di risultati e la salvo come dataframe
res = as.data.frame(t(res))

# Bindo il dataset di dati grezzi e il dataset corrispondente di risultati
ITA = bind_cols(ITA, res)
...
# rinomonio la colonna nel modo giusto
ITA = ITA %>% rename(qvalue = res)

# reorder
ITA = ITA[, c(1:6, 10, 7:9)]

# Ordinare e subsettare link
# http://adv-r.had.co.nz/Subsetting.html
ITA = ITA[order(ITA$p.value, ITA$qvalue),]
```

```{r}
write.xlsx(ITA, file = paste(gender.path, "ITA by Gender.xlsx", sep = ""))
```
